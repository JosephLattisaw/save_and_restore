...
class HostServer {
public:
    HostServer(boost::asio::io_service &io_service, std::uint16_t port,
...
private:
    ...
    public:
        ...
        HostServerClient(std::uint64_t client_number, boost::asio::io_service &io_service,
                         std::unique_ptr<boost::asio::ip::tcp::socket> sock,
                         Disconnect_Callback callback);

        void send_application_update(std::vector<std::string> application_names,
                                     shadow::ApplicationStatuses application_statuses);
    ...
    private:
        ...
        void start_read();
        void reset_buffers();
        void write_error_handler(const boost::system::error_code &error,
